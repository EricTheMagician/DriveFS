...
ChannelsApi::ChannelsApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<void> ChannelsApi::stop(utility::string_t alt, utility::string_t fields, utility::string_t key, utility::string_t oauthToken, bool prettyPrint, utility::string_t quotaUser, utility::string_t userIp, std::shared_ptr<Channel> resource)
{


    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/channels/stop");
    ...
    {
        throw ApiException(400, U("ChannelsApi->drive.channels.stop does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("ChannelsApi->drive.channels.stop does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling drive.channels.stop: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling drive.channels.stop: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
